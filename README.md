...
* **Permission Control**: Require players to have a specific permission node (specialitem.use.\<id\>) to use an item.  
...
3. Place the SpecialItem-vX.X.X.jar file into your server's /plugins directory.  
4. Start or restart your server. A SpecialItem folder containing the config.yml file will be generated.  
...
items:  
  fire\_sword:  
    ...
    \# Enchantments: ENCHANTMENT\_NAME: level  
    enchantments:  
      FIRE\_ASPECT: 2  
      SHARPNESS: 7  
    ...
    \# If true, player needs 'specialitem.use.fire\_sword' permission.  
...
| :---- | :---- | :---- |
| /si reload | Reloads the plugin's configuration. | specialitem.admin |
| /si get \<id\> \[amount\] | Gives the player a specified special item. | specialitem.admin |
| /si | Shows the help message. | specialitem.admin |

**Player Permissions:**

* specialitem.use.\<item\_id\>: Allows a player to use the special item with the corresponding ID. For example, specialitem.use.fire\_sword.  
* specialitem.use.\*: Allows a player to use all special items.
...
| :---- | :---- | :---- |
| %specialitem\_cooldown\_\<item\_id\>% | Shows the formatted remaining cooldown for an item. | 1m 15s / Ready |
| %specialitem\_cooldown\_s\_\<item\_id\>% | Shows the remaining cooldown in seconds. | 75 / 0 |
| %specialitem\_name\_\<item\_id\>% | Shows the MiniMessage-formatted name of the item. | Infernal Blade |
...
**SpecialItem** 是一款功能强大、高度可定制的 Minecraft 插件，专为 Purpur 服务端设计。它允许服主创建拥有特殊能力的独特物品。这些物品可以执行指令、拥有冷却时间、需要特定权限等。所有特殊属性都通过持久化数据容器（PDC）直接存储在物品本身，确保物品在交易或存放后仍能保持其独特性。
...
* **权限控制**: 可要求玩家拥有特定的权限节点 (specialitem.use.\<id\>) 才能使用物品。  
* **可配置行为**:  
...
3. 将下载的 SpecialItem-vX.X.X.jar 文件放入服务器的 /plugins 文件夹中。  
4. 启动或重启服务器。此时会自动生成一个 SpecialItem 文件夹，其中包含 config.yml 配置文件。  
...
items:  
  fire\_sword:  
    ...
    \# 若为 true，玩家需要 'specialitem.use.fire\_sword' 权限才能使用。  
    permission-required: true  
    \# 冷却时间（秒）。  
    cooldown: 60  
    \# 若为 true，每次使用会消耗一个物品。  
...
| 指令 | 描述 | 权限 |
| :---- | :---- | :---- |
| /si reload | 重载插件的配置文件。 | specialitem.admin |
| /si get \<id\> \[数量\] | 给予玩家一个指定的特殊物品。 | specialitem.admin |
| /si | 显示帮助信息。 | specialitem.admin |

**玩家权限:**

* specialitem.use.\<item\_id\>: 允许玩家使用对应ID的特殊物品。例如 specialitem.use.fire\_sword。  
* specialitem.use.\*: 允许玩家使用所有特殊物品。
...
| 变量 | 描述 | 输出示例 |
| :---- | :---- | :---- |
| %specialitem\_cooldown\_\<item\_id\>% | 显示一个物品格式化后的剩余冷却时间。 | 1m 15s / 可用 |
| %specialitem\_cooldown\_s\_\<item\_id\>% | 显示剩余冷却时间的秒数。 | 75 / 0 |
| %specialitem\_name\_\<item\_id\>% | 显示物品经过 MiniMessage 格式化后的名称。 | 炼狱之刃 |
